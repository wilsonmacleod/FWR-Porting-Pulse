...
def insert_sql(perc,select_month):
    ...
    conn = psycopg2.connect(database = pg_sql.db_name, user = pg_sql.postgres_user, password = pg_sql.postgres_pw)
    cur = conn.cursor()
    query = """
...
def main(select_month):
    ...
    datelist = []
    for date in get_wildcard(span,start_date): #run query for each day (ports crd on that day are the ones queried)
        wildcard = date
